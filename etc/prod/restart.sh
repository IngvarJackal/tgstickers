...
echo 'while true; do sleep 60; if [[ "OK" == $(curl localhost:10000 2> /dev/null) ]] ; then echo $(date) "OK"; else echo $(date) "NOK"; ./docker-compose -f docker-compose-prod.yml restart inservice; done' > inservice_keeper.sh; fi'
echo 'while true; do sleep 60; if [[ "OK" == $(curl localhost:10001 2> /dev/null) ]] ; then echo $(date) "OK"; else echo $(date) "NOK"; ./docker-compose -f docker-compose-prod.yml restart blservice; done' > blservice_keeper.sh; fi'
echo 'while true; do sleep 60; if [[ "OK" == $(curl localhost:10002 2> /dev/null) ]] ; then echo $(date) "OK"; else echo $(date) "NOK"; ./docker-compose -f docker-compose-prod.yml restart outservice; done' > outservice_keeper.sh; fi'
