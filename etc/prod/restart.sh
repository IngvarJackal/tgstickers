...
echo 'while true; do sleep 60; [[ "OK" == $(curl localhost:10000 2> /dev/null) ]] && echo $(date) "OK" || echo $(date) "NOK"; ./docker-compose -f docker-compose-prod.yml restart inservice; done' > inservice_keeper.sh
echo 'while true; do sleep 60; [[ "OK" == $(curl localhost:10001 2> /dev/null) ]] && echo $(date) "OK" || echo $(date) "NOK"; ./docker-compose -f docker-compose-prod.yml restart blservice; done' > blservice_keeper.sh
echo 'while true; do sleep 60; [[ "OK" == $(curl localhost:10002 2> /dev/null) ]] && echo $(date) "OK" || echo $(date) "NOK"; ./docker-compose -f docker-compose-prod.yml restart outservice; done' > outservice_keeper.sh
